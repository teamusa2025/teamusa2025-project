...
export default function FCModal({
  ...
  onSave,
  onClose,
}: FCModalProps): JSX.Element {
  const [forecastConfig, setForecastConfig] = useState<
  Record<string, RowConfig>
  >(() => {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-50 flex items-center justify-center bg-gray-600/50 p-6"
        initial="hidden"
        animate="visible"
        exit="hidden"
      ...
      >
        <motion.div
          className="flex max-h-[90vh] w-full max-w-5xl flex-col rounded-lg bg-white p-6 shadow-lg dark:bg-gray-900"
          initial="hidden"
          animate="visible"
          exit="hidden"
          ...
          <div className="grid grid-cols-4 gap-6">
            {rowsToEdit.map((displayName) => {
              ...
              return (
                <div
                  key={key}
                  className="flex flex-col space-y-2 border-b border-gray-200 px-2 pb-2"
                >
